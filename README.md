...
| `/rfps/:rfpId/:section?`                                            | **Working, read-only.** The RFP workspace, with tabs for Overview, Sources, Requirements, Fit review, Decision, Activity, and Upcoming. Each tab has its own URL, and Back returns to the pipeline. Three RFPs have detailed fixtures; the rest show honest empty states. Unknown IDs show "RFP not found". |
